...
export const createOrder = async (userId, shippingAddress, paymentMethod) => {
  ...
  const normalizedMethod = (paymentMethod || 'razorpay').toLowerCase();
  const methodValue = normalizedMethod === 'cod' ? 'COD' : 'razorpay';
  const statusValue = normalizedMethod === 'cod' ? 'cod' : 'pending';
